...
def test_compute_meta_features():
    ...
    # 15 samples, 1 base classifier, 2 classes
    meta_test.dsel_scores_ = dsel_scores_ex1[:, 0, :].reshape(15, 1, 2)
...
# Test the estimate competence function considering 3 base classifiers and
# 1 test sample
...
def test_estimate_competence_batch():
    ...
    meta_test._get_similar_out_profiles = MagicMock(
        return_value=(None, neighbors_ex1[:, 0:meta_test.Kp]))
...
def test_sample_selection():
    pool_classifiers = create_pool_all_agree(0, 10) + \
        create_pool_all_agree(1, 5)
