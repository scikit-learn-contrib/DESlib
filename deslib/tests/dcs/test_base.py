...
def test_valid_selection_method(selection_method, create_pool_classifiers):
    ...
    with pytest.raises(ValueError):
        dcs = BaseDCS(create_pool_classifiers, selection_method=selection_method)
        dcs.fit(X, y)
...
def test_valid_diff_threshold_type(diff_thresh, create_pool_classifiers):
    ...
    y = np.ones(10)
    with pytest.raises(TypeError):
        dcs = BaseDCS(create_pool_classifiers, selection_method='diff', diff_thresh=diff_thresh)
        dcs.fit(X, y)
...
def test_select_diff(competences, expected):
    ...
    dcs_test = BaseDCS(selection_method='diff', diff_thresh=0.15, random_state=rng)
...
@pytest.mark.parametrize('competences, expected', [([0.6, 0.2, 0.6], [0.415, 0.585]),
                                                   ([0.5, 0.8, 0.5], [1.0, 0.0])])
def test_predict_proba_instance_all(competences, expected, create_pool_classifiers):
    query = np.array([-1, 1])
    pool_classifiers = create_pool_classifiers
    dcs_test = BaseDCS(pool_classifiers, selection_method='all')
    dcs_test.n_classes_ = 2
