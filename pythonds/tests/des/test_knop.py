...
def test_estimate_competence(index, expected):
    ...
    knop_test.mask = np.ones(knop_test .n_classifiers)
...
def test_fit():
    ...
    # Assert the knn is fitted to the scores (decision space) rather than the features (feature space)
    assert np.array_equal(knop_test.knn._fit_X, knop_test.dsel_scores)
