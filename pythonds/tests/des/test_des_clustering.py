...
from pythonds.tests.examples_test import create_pool_classifiers, X_dsel_ex1, y_dsel_ex1
from pythonds.util.diversity import Q_statistic, ratio_errors, negative_double_fault
...
def test_fit_homogeneous_clusters():
    ...
    clustering_test.clustering.fit_predict = MagicMock(return_value=return_cluster_index_ex1)

    clustering_test.mask = np.ones(clustering_test.n_classifiers)
...
def test_fit_heterogeneous_clusters():
    ...
    clustering_test.clustering.fit_predict = MagicMock(return_value=return_cluster_index_ex2)
...
def test_estimate_competence():
    ...
    clustering_test.mask = np.ones(clustering_test.n_classifiers)
    clustering_test.fit(X_dsel_ex1, y_dsel_ex1)
    clustering_test.clustering.predict = MagicMock(return_value=0)
    ...
    clustering_test.clustering.predict = MagicMock(return_value=1)
    competences = clustering_test.estimate_competence(query)
